<?php
    ...
    // CHECK FRIENDS
    $is_already_friends = $frnd_obj->is_already_friends($_SESSION['user_id'], $user_data->id);   //tra ve true/false
    //  IF I AM THE REQUEST SENDER
    $check_req_sender = $frnd_obj->am_i_the_req_sender($_SESSION['user_id'], $user_data->id);    //tra ve true/false
    // IF I AM THE REQUEST RECEIVER
    $check_req_receiver = $frnd_obj->am_i_the_req_receiver($_SESSION['user_id'], $user_data->id);//tra ve true/false
    // TOTAL REQUESTS
    $get_req_num = $frnd_obj->request_notification($_SESSION['user_id'], false);     //tra ve 1 số(so hang)
    // TOTAL FRIENDS
    $get_frnd_num = $frnd_obj->get_all_friends($_SESSION['user_id'], false);         //tra ve 1 số(so hang)
?>
<!DOCTYPE html>
<html lang="en">
<head>
