...
class Friend{ 
    ...
    // CHECK IF ALREADY FRIENDS =>tra ve true/false
    ...
    //  IF I AM THE REQUEST SENDER =>tra ve true/false
    ...
    //  IF I AM THE RECEIVER =>tra ve true/false
    ...
    // CHECK IF REQUEST HAS ALREADY BEEN SENT(KIỂM TRA NẾU YÊU CẦU ĐÃ ĐƯỢC GỬI) =>tra ve true/false
    ...
    // MAKE PENDING FRIENDS (SEND FRIEND REQUEST)=>gui yeu cầu kết bạn
    ...
    // CANCLE FRIEND REQUEST(Huy yeu cau ket ban)
    ...
    public function make_friends($my_id, $user_id){
        ...
        try{
            ...
            $delete_stmt->execute();
            if($delete_stmt->execute()){//neu dung
    ...
    // REQUEST NOTIFICATIONS=>tra ve so thong bao
    public function request_notification($my_id, $send_data){
        try{
            ...
            $stmt->execute([$my_id]);
            if($send_data){
                return $stmt->fetchAll(PDO::FETCH_OBJ);//tra ve mang
            }
            else{
                return $stmt->rowCount();//tra ve số thông báo
            }
        }
        catch (PDOException $e) {
            die($e->getMessage());
    ...
    //lấy tất cả friend=>tra ve 1 mang
